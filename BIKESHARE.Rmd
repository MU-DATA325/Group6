...
```{r bar-graph-by-gender-September-2019 NY19}
...
```{r line-graph-tripduration-usertype-September-2019 NY19}
ggplot(data = NY19) + 
...
```{r glimpse-of-dataset-September-2020 NY20}
glimpse (NYtripdata20)
```


```{r creating-age-variable-September-2020 NY20}
 NY20 <-NYtripdata20 %>% 
...
```{r bar-graph-by-gender-September-2020 NY20}
...
```{r calculate-statistics-for-trip duration-by-gender-September-2020 NY20}
NY20 %>%
  group_by(gender) %>%
 summarise(minimum_trdur=min(tripduration),mean_trdur=mean(tripduration),median_trdur=median(tripduration), 
            maximum_trdur=max(tripduration))
...
```{r line-graph-tripduration-usertype-September-2020 NY20}
ggplot(data = NY20) + 
  geom_line(mapping = aes(x = tripduration, y = usertype))  
...
```{r count-last-ten-less-used-bikes-for-September-2020 NY20}
NY20 %>%
  count(bikeid)%>%
  arrange(desc(n))%>%
          slice_tail(n=10)
...
```{r glimpse-at-trips-dataframe-Washington-DC-last-quarter-2020 NY20}
glimpse(trips)
```
