...
```{r calculate-statistics-for-tripduration-in-minutes-by-agegroup-September-2019}
NY19 %>%
  group_by(agegroup) %>%
 summarise(minimum_trdur=min(tripduration.min),mean_trdur=mean(tripduration.min),median_trdur=median(tripduration.min), 
            maximum_trdur=max(tripduration.min))
...
```{r column-graph-agegroup-tripduration-usertype-September-2019}
ggplot(data = NY19) + 
...
NY20  <- mutate(NY20, tripduration.min = tripduration/60)
x.max <- quantile(NY20$tripduration.min, 0.99)
```
```{r defining-tripduration.min-as-integer-September-2020}
NY20$tripduration.min <- as.integer(NY20$tripduration.min)
```
